...
Vecteur3D position1 (3.01, 0, 0); //constructeur directe ?
...
int main()
{
    ...
    for(size_t i(0); i < 20; i+=5) {
        ...
        Quadrupole* q1(new Quadrupole(sup, entrees[i], sorties[i], re, b1));
        Section_droite* s1(new Section_droite(sup, entrees[i+1], sorties[i+1],re));
        Quadrupole* q2(new Quadrupole(sup, entrees[i+2], sorties[i+2], re, b2));
        Section_droite* s2(new Section_droite(sup, entrees[i+3], sorties[i+3],re));
        Dipole* d1(new Dipole(sup, entrees[i+4],sorties[i+4], re, k,b3));
    ...
    acc.vis_element_precedent();

    Particule reference1( sup, position1,axe1,force1,0.938272, 1.60217653e-19,2, (acc.get_collection_el())[2]);
    Particule reference2( sup, position2,axe2,force2,0.938272, 1.60217653e-19, 2, (acc.get_collection_el())[2]);

    Faisceau faisceau1(reference1, 100, 1, 1);
    Faisceau faisceau2(reference2, 200, 1, 1e-11);
